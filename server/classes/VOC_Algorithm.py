...
class DFRobot_VOCAlgorithm:
    ...
    def _vocalgorithm__init_instances(self):
        ...
        self._vocalgorithm__mean_variance_estimator__set_parameters(self._f16(VOCALGORITHM_SRAW_STD_INITIAL),
    ...
    def _vocalgorithm_set_states(self, state0, state1):
        self._vocalgorithm__mean_variance_estimator__set_states(self.params, state0, state1,
                                                                self._f16(VOCALGORITHM_PERSISTENCE_UPTIME_GAMMA))
        self.params.msraw = state0

    def _vocalgorithm_set_tuning_parameters(self, voc_index_offset, learning_time_hours, gating_max_duration_minutes,
                                            std_initial):
    ...
    def _vocalgorithm__mean_variance_estimator__set_parameters(self, std_initial, tau_mean_variance_hours,
        ...
        self.params.m_mean_variance_estimator_gamma = self._fix16_div(
            self._f16((VOCALGORITHM_MEAN_VARIANCE_ESTIMATOR__GAMMA_SCALING * (VOCALGORITHM_SAMPLING_INTERVAL / 3600.)) \
    ...
    def _vocalgorithm__mean_variance_estimator___calculate_gamma(self, voc_index_from_prior):
        ...
        gating_threshold_mean = (self._f16(VOCALGORITHM_GATING_THRESHOLD) \
                                 + (self._fix16_mul(
                    self._f16((VOCALGORITHM_GATING_THRESHOLD_INITIAL - VOCALGORITHM_GATING_THRESHOLD)), \
                    self._vocalgorithm__mean_variance_estimator___sigmoid__process(
                        self.params.m_mean_variance_estimator_uptime_gating))))
        self._vocalgorithm__mean_variance_estimator___sigmoid__set_parameters(self._f16(1.), gating_threshold_mean,
                                                                              self._f16(
        ...
        self._vocalgorithm__mean_variance_estimator___sigmoid__set_parameters(self._f16(1.), gating_threshold_variance,
                                                                              self._f16(
                                                                                  VOCALGORITHM_GATING_THRESHOLD_TRANSITION))
        ...
        if ((self.params.m_mean_variance_estimator_initialized == 0)):
            ...
            self.params.m_mean_variance_estimator_std = self._fix16_mul(
                self._fix16_sqrt((self._fix16_mul(additional_scaling, \
                                                  (self._f16(
                                                      VOCALGORITHM_MEAN_VARIANCE_ESTIMATOR__GAMMA_SCALING) - self.params.m_mean_variance_estimator__gamma_variance)))), \
    ...
    def _vocalgorithm__mean_variance_estimator___sigmoid__init(self):
        self._vocalgorithm__mean_variance_estimator___sigmoid__set_parameters(self._f16(0.), self._f16(0.),
                                                                              self._f16(0.))
    ...
    def _vocalgorithm__mox_model__process(self, sraw):
        return (self._fix16_mul((self._fix16_div((sraw - self.params.m_mox_model_sraw_mean), (
            -(self.params.m_mox_model_sraw_std + self._f16(VOCALGORITHM_SRAW_STD_BONUS))))),
                                self._f16(VOCALGORITHM_VOC_INDEX_GAIN)))
    ...
    def _vocalgorithm__sigmoid_scaled__process(self, sample):
        ...
        if ((x < self._f16(-50.))):
            ...
            if ((sample >= self._f16(0.))):
                shift = (self._fix16_div((self._f16(VOCALGORITHM_SIGMOID_L) - (
                    self._fix16_mul(self._f16(5.), self.params.m_sigmoid_scaled_offset))), self._f16(4.)))
                return ((self._fix16_div((self._f16(VOCALGORITHM_SIGMOID_L) + shift),
                ...
                return (self._fix16_mul((self._fix16_div(self.params.m_sigmoid_scaled_offset,
                                        ...
                                        (self._fix16_div(self._f16(VOCALGORITHM_SIGMOID_L),
    ...
    def _vocalgorithm__adaptive_lowpass__process(self, sample):
        ...
        tau_a = ((self._fix16_mul(self._f16((VOCALGORITHM_LP_TAU_SLOW - VOCALGORITHM_LP_TAU_FAST)), F1)) + self._f16(
