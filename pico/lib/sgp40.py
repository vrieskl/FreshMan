...
class SGP40:
    ...
    def measure_raw(self, humidity=50, temperature=25):
        param_h = struct.pack(">H", math.ceil(humidity * 0xffff / 100))
        crc_h = self.__crc(param_h[0], param_h[1])
        param_t = struct.pack(">H", math.ceil((temperature + 45) * 0xffff / 175))
        crc_t = self.__crc(param_t[0], param_t[1])
        data = param_h + bytes([crc_h]) + param_t + bytes([crc_t])
    ...
    def __write_command(self, cmd):
        b_cmd = struct.pack('>H', cmd)
        self.i2c.writeto(self.addr, b_cmd)
